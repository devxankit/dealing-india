...
const RealEstateDevelopers = () => {
    ...
    // Get unique cities from developers
    const cities = useMemo(() => {
        const extracted = developers.map(dev => dev.location.split(',')[0].trim());
    ...
    const filteredDevelopers = useMemo(() => {
        return developers.filter(dev => {
            // City check
            const devCity = dev.location.split(',')[0].trim();
            const matchesCity = selectedCity === 'All Cities' || devCity === selectedCity;

            // Search logic: Name, Location, or Seller
            const matchesSearch =
                dev.projectName.toLowerCase().includes(searchQuery.toLowerCase()) ||
                dev.location.toLowerCase().includes(searchQuery.toLowerCase()) ||
                dev.sellerName.toLowerCase().includes(searchQuery.toLowerCase());

            // Price Filter logic: treated as Lakhs
            ...
            const propertyMinPrice = dev.minPriceValue || 0;
            const propertyMaxPrice = dev.maxPriceValue || propertyMinPrice;

            const matchesPrice = propertyMaxPrice >= minVal && propertyMinPrice <= maxVal;
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-20">
            <B2BHeader title="Developer Properties" />
            ...
            <main className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-10">
                    ...
                    <div className="flex-[2] relative group">
                        ...
                        <input
                            type="text"
                            placeholder="Search project name, location or developer..."
                            value={searchQuery}
                    ...
                    <div className="relative">
                        ...
                        >
                            <FiFilter />
                            <span>{(appliedPrice.min || appliedPrice.max) ? 'Price Active' : 'Budget'}</span>
                        </button>
                        ...
                        <AnimatePresence>
                            {isPriceFilterOpen && (
                                ...
                                >
                                    <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Price Range (In Lakhs)</h4>
                ...
                <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tight leading-none mb-2">Direct from <span className="text-primary-600">Developers</span></h2>
                        <p className="text-gray-500 text-sm font-medium tracking-wide uppercase">
                            {filteredDevelopers.length} Properties in {selectedCity}
                        </p>
                    </div>
                    {/* Active Filter Badges */}
                    <div className="flex flex-wrap items-center gap-2">
                        {selectedCity !== 'All Cities' && (
                            <span className="px-3 py-1 bg-primary-50 text-primary-600 text-[10px] font-black uppercase rounded-full border border-primary-100 flex items-center gap-2">
                ...
                {loading ? (
                    ...
                    >
                        {filteredDevelopers.map(dev => (
                            <RealEstateCard key={dev.id} property={dev} />
                        ))}
                    </motion.div>
                ) : (
