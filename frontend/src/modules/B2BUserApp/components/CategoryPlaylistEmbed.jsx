...
export default function CategoryPlaylistEmbed({ categoryName, title = 'Videos from this category', className = '' }) {
  ...
  return (
    <div className={`rounded-2xl border border-gray-100 bg-white overflow-hidden shadow-sm ${className}`}>
      <div className="p-4 border-b border-gray-100 flex items-center gap-2">
        <FiVideo className="text-primary-600" />
        <h3 className="font-bold text-gray-900">{title}</h3>
      </div>
      <div className="aspect-video max-h-[360px] bg-gray-900">
        <iframe
          title={`YouTube playlist: ${categoryName}`}
