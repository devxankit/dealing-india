...
const WholesalerMessages = () => {
    ...
    const selectConversation = (conv) => {
        ...
        setMessages([
            { _id: 'm1', senderId: 'v123', text: conv.lastMessage, time: '10:30 AM' },
            { _id: 'm2', senderId: 'w123', text: 'Hello! Our MOQ is 100 units.', time: '10:35 AM' },
        ]);
    };

    const handleSend = () => {
        if (!messageText.trim()) return;
        const newMessage = { _id: Date.now().toString(), senderId: 'w123', text: messageText, time: 'Just now' };
            ...
            <div className={`${selectedConversation ? 'hidden md:flex' : 'flex'} w-full md:w-80 border-r border-gray-100 flex-col`}>
                <div className="p-4 border-b border-gray-50">
                    ...
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Search vendors..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-sm" />
                    </div>
                </div>
                ...
                {selectedConversation ? (
                    <>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            {messages.map(msg => {
                                const isWholesaler = msg.senderId === 'w123';
                                return (
                                    <div key={msg._id} className={`flex ${isWholesaler ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[70%] p-3 rounded-2xl ${isWholesaler ? 'bg-primary-600 text-white rounded-tr-none' : 'bg-white text-gray-800 shadow-sm rounded-tl-none'}`}>
                                            <p className="text-sm">{msg.text}</p>
                                            <span className={`text-[10px] block mt-1 ${isWholesaler ? 'text-primary-100' : 'text-gray-400'}`}>{msg.time}</span>
                                        </div>
                                    </div>
                                );
                            })}
...
export default WholesalerMessages;
