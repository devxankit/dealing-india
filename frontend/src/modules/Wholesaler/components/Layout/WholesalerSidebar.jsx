...
import wholesalerMenu from "../../config/wholesalerMenu.json";
...
const iconMap = {
    ...
    "Wholesale Inquiries": FiMessageCircle,
    Vendors: FiUsers,
...
const getChildRoute = (parentRoute, childName) => {
    const routeMap = {
        "/wholesaler/products": {
            "Manage Products": "/wholesaler/products/manage-products",
            "Add Product": "/wholesaler/products/add-product",
        },
        "/wholesaler/settings": {
            "Profile": "/wholesaler/settings/profile",
            "Business Details": "/wholesaler/settings/business",
            "Security": "/wholesaler/settings/security",
        },
    };
    return routeMap[parentRoute]?.[childName] || parentRoute;
};

const WholesalerSidebar = ({ isOpen, onClose }) => {
    const location = useLocation();
    const navigate = useNavigate();
    // Placeholder for wholesaler auth - will replace with real store later
    const wholesaler = { name: "Wholesaler Store", email: "wholesaler@example.com" };
    const [expandedItems, setExpandedItems] = useState({});

    useEffect(() => {
        const activeItem = wholesalerMenu.find((item) => {
            if (item.route === "/wholesaler/dashboard") {
                return location.pathname === "/wholesaler/dashboard";
            }
    ...
    const isActive = (route) => {
        if (route === "/wholesaler/dashboard") return location.pathname === "/wholesaler/dashboard";
    ...
    const sidebarContent = (
        <div className="h-full flex flex-col bg-slate-800 shadow-xl">
                ...
                <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                            <h2 className="font-semibold text-white text-sm truncate">{wholesaler.name}</h2>
                            <p className="text-xs text-gray-400 truncate">{wholesaler.email}</p>
                        </div>
                    </div>
            ...
            <nav className="flex-1 overflow-y-auto p-3 scrollbar-admin">
                {wholesalerMenu.map(renderMenuItem)}
            </nav>
        </div>
    );
