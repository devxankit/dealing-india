...
const WholesalerHeader = ({ onMenuClick }) => {
    const location = useLocation();
    const navigate = useNavigate();
    // Placeholder for wholesaler auth
    const wholesaler = { name: "Wholesaler Store", email: "wholesaler@example.com" };
    const logout = () => { };
                    ...
                    <div className="relative" ref={userMenuRef}>
                        ...
                        >
                            ...
                            <div className="hidden sm:block text-left mr-1">
                                <p className="text-sm font-bold text-gray-800 leading-none mb-0.5">{wholesaler.name}</p>
                                <p className="text-[10px] text-gray-500 font-medium leading-none">Wholesaler Account</p>
                            </div>
                            ...
                            {showUserMenu && (
                                ...
                                >
                                    <div className="px-4 py-3 border-b border-gray-50 mb-1">
                                        <p className="text-sm font-bold text-gray-900 truncate">{wholesaler.name}</p>
                                        <p className="text-xs text-gray-500 truncate mt-0.5">{wholesaler.email}</p>
                                    </div>
                                    <Link to="/wholesaler/settings/profile" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 hover:text-primary-600 transition-colors">
                                        <FiUser className="text-lg" />
                                        <span className="font-medium">My Profile</span>
                                    </Link>
                                    <Link to="/wholesaler/settings/business" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 hover:text-primary-600 transition-colors">
