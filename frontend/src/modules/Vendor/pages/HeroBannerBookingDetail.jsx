...
import { getVendorBannerBookingDetails } from "../../Vendor/services/heroBannerService";

const VendorHeroBannerBookingDetail = () => {
    const { id } = useParams();
    ...
    const loadBooking = async () => {
        setLoading(true);
        try {
            const response = await getVendorBannerBookingDetails(id);
            setBooking(response.data);
        } catch (error) {
            console.error("Error loading booking details:", error);
            toast.error("Failed to load booking details");
            navigate("/vendor/hero-banner-booking");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto">
            ...
            >
                <FiArrowLeft /> Back to My Banners
            </button>
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-8 items-start justify-between">
                ...
                {booking.status === "pending" && (
                    <div className="bg-yellow-50 text-yellow-800 px-4 py-3 rounded-lg flex items-center gap-3 border border-yellow-200">
                        <FiAlertCircle className="text-xl flex-shrink-0" />
                        <div>
                            <p className="font-medium text-sm">Processing Approval</p>
                            <p className="text-xs opacity-90">Your banner is currently being reviewed by our admin team.</p>
                        </div>
                    </div>
                )}
            </div>
                ...
                <div className="space-y-6">
                    ...
                    {booking.adminApprovalStatus === 'rejected' && (
                        <div className="bg-red-50 rounded-xl shadow-sm border border-red-100 p-6">
                            ...
                            <p className="text-red-700">{booking.rejectionReason || "No reason provided. Please contact support."}</p>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-bold text-gray-900">{formatPrice(booking.amount)}</span>
                                </div>
...
export default VendorHeroBannerBookingDetail;
