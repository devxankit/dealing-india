    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          {/* Header */}
          <div className={`${meta.bg} p-7 flex items-center justify-between`}>
...
const AdminTransactions = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/transactions?type=${activeTab}&page=${page}&limit=${LIMIT}`);
      if (res.success) {
              ...
              return (
                <button key={tab.key}
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-black ${
                    activeTab === tab.key ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-500'
                  }`}>
                    {tab.key === 'all' ? total :
                    ...
                    <motion.tr key={txn._id || idx}
                      ...
                      <td className="px-6 py-4 text-center">
                        <button
                          ...
                          className="p-2 rounded-xl bg-gray-50 hover:bg-indigo-50 hover:text-indigo-600 text-gray-400 transition-all opacity-0 group-hover:opacity-100">
                          <FiEye size={16} />
                        </button>
                      </td>
                    </motion.tr>
